...
## 29. 결제 및 환불 내역 상세 보기 구현

- [x] 테이블 로우 클릭 이벤트 및 상태 관리 구현
- [x] 영수증 스타일의 상세 내역 모달 구현
- [x] 레이아웃 겹침 문제 해결 및 디자인 고도화
- [x] 모달 높이 및 패딩 수정을 통한 UI 최적화
- [x] 다크 모드 및 반응형 대응 확인

## 30. 이용약관 및 개인정보처리방침 페이지 구현

- [x] 이용약관(`Terms.tsx`) 및 개인정보처리방침(`Privacy.tsx`) 컴포넌트 구현
- [x] `AppRouter.tsx` 내 신규 라우트 등록 (`/terms`, `/privacy`)
- [x] `Footer.tsx` 내 정적 텍스트를 `Link` 또는 `navigate`로 변경 및 연결
- [x] 텍스트 기반의 깔끔한 법적 고지 UI 디자인 및 반응형 대응

## 31. 백엔드 API 응답 구조 변경 대응 (BaseResponse 도입)

- [x] `src/types/common.ts` 내 `BaseResponse<T>` 인터페이스 정의
- [x] `AccessLog.tsx` 내 API 호출 및 데이터 매핑 로직 수정 (`BaseResponse` 적용)
- [x] 기타 실데이터 연동 중인 관리자 페이지 점검 및 필요 시 수정

## 32. 네비게이션 메뉴 경로 연결 (공방 탐색)

- [x] `Header.tsx` 내 '카테고리별 클래스' 메뉴 경로를 `/workshops`로 연결
- [ ] 기타 미연결 메뉴(실시간 핫플레이스, 신규 공방 등) 점검

## 33. 장바구니 > 결제하기 팝업(주문서) 구현

- [x] `CartPage` 내 '결제하기' 버튼 클릭 시 결제 모달 오픈 로직 추가
- [x] 결제 모달(`CheckoutModal`) UI 구현 (주문 내역 요약, 결제 수단 선택)
- [x] 등록된 결제 카드 리스트 연동 및 선택 기능 구현
...
## 35. 시스템 관리 > 접속 이력 관리 UI 고도화

- [x] '로그 내보내기 (CSV)' 버튼 제거
- [x] '새로고침' 버튼 추가 및 스타일 통일
...
## 37. 관리자 API 호출 로직 분리 및 리팩토링

- [x] `UserAccessLogList` 타입 이동 (`src/types/user.ts`)
- [x] `adminService.ts` 신규 생성 및 API 호출 로직 통합
...
- [x] 사용자 등록 모달 컴포넌트(`UserCreateModal.tsx`) 생성 (Signup UI 재사용)
- [x] `UserManagement` 페이지 내 등록 모달 연동 및 상태 관리
- [x] 등록 성공 시 목록 새로고침 로직 연결
- [x] 최종 테스트 및 예외 처리

## 40. 시스템 관리 > 사용자 등록 모달 UI 최적화

- [x] 모달 높이 및 내부 간격 조정을 통한 스크롤 제거
- [x] 반응형 레이아웃 재점검

## 41. 내 정보 관리 기능 구현

- [x] `UserViewResponse` 타입 정의 및 `userService.ts` 생성
- [x] 내 정보 관리 페이지(`Profile.tsx`) UI 개발
- [x] `/api/user/me` API 연동 및 데이터 바인딩
- [x] 헤더 및 사용자 메뉴 내 '내 정보 관리' 링크 연결
- [x] 디자인 고도화 (카드 레이아웃 및 다크 모드 대응)

## 42. 헤더 UI 텍스트 수정 및 최적화

- [x] '주문 대기함' → '장바구니'로 텍스트 변경
- [x] 사용자 메뉴 내 'My Account' 라벨 제거

## 43. 헤더 드롭다운 외부 클릭 시 닫기 기능 구현
- [x] `useRef`를 이용한 드롭다운 영역 참조 설정
- [x] 외부 클릭 감지 이벤트 리스너 등록 및 해제 로직 추가
- [x] 예약/관심/장바구니/사용자 메뉴 전체 드롭다운에 적용
- [x] 메뉴 간 전환 시 충돌 방지 로직 보완

## 44. 시스템 관리 > 게시판 관리 기능 구현
- [x] 게시판 관리 관련 타입 정의 (`types/board.ts`)
- [x] `adminService` 내 게시판 목록 조회 API 연동
...
## 47. 시스템 관리 > 게시판 수정 기능 구현 (모달형)
- [x] 게시판 수정 모달 컴포넌트 (`BoardEditModal.tsx`) 껍데기 구현
- [x] 관리 테이블에 수정 아이콘 추가 및 모달 호출 연동
- [x] 모달 오픈 시 기존 데이터 바인딩
