        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300 px-6">
            ...
            >
                ...
                <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between flex-shrink-0">
                    <div>
                        <h2 className="text-xl font-black text-slate-900 dark:text-white">{t('cart.modalTitle')}</h2>
                        <p className="text-xs text-slate-400 font-medium mt-0.5">{t('cart.modalDesc')}</p>
                    </div>
                    <button 
                        onClick={onClose} 
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                    {status === 'SUCCESS' ? (
                        <div className="py-12 text-center animate-in zoom-in duration-500">
                            ...
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2">{t('cart.paymentSuccess')}</h3>
                            <p className="text-slate-500 dark:text-slate-400 font-medium mb-8">{t('cart.redirectingToReservations')}</p>
                        </div>
                    ) : (
                        <div className="space-y-8">
                            ...
                            <section>
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">{t('cart.orderSummary')}</h4>
                                    <span className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{t('cart.itemCount', { count: cartItems.length })}</span>
                                </div>
                            ...
                            <section>
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">{t('cart.selectPaymentMethod')}</h4>
                                    <button 
                            ...
                            <section className="bg-slate-50 dark:bg-slate-800/50 rounded-3xl p-6 border border-slate-100 dark:border-slate-800">
                                <div className="space-y-3">
                                    <div className="flex justify-between text-sm">
                                        <span className="text-slate-500 font-medium">{t('cart.totalProductAmount')}</span>
                                        <span className="text-slate-900 dark:text-white font-bold">₩{totalPrice.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between text-sm">
                                        <span className="text-slate-500 font-medium">{t('cart.discountAmount')}</span>
                                    ...
                                    <div className="flex justify-between items-end">
                                        <span className="text-sm font-black text-slate-800 dark:text-slate-200">{t('cart.finalPrice')}</span>
                            ...
                            <div className="flex items-center gap-2 px-4 py-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-blue-600 dark:text-blue-400">
                                ...
                                <p className="text-[10px] font-bold leading-tight">{t('cart.agreeNotice')}</p>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <div className="px-8 py-6 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                        ...
                        >
                            {status === 'PROCESSING' ? (
                                ...
                                <>
                                    <span>{t('cart.payAmount', { amount: totalPrice.toLocaleString() })}</span>
