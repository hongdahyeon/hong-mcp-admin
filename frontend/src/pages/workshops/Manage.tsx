...
const WorkshopManage: React.FC = () => {
    ...
    const handleDelete = (id: string) => {
        if (window.confirm(t('workshopManage.confirmDelete'))) {
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div>
                    ...
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">{t('workshopManage.title')}</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">{t('workshopManage.subtitle')}</p>
                </div>
                <Link 
                ...
                >
                    <Plus size={20} /> {t('workshopManage.newClassBtn')}
                </Link>
            </div>
            ...
            {myWorkshops.length === 0 ? (
                <div className="bg-white dark:bg-slate-950 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[2.5rem] py-24 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{t('workshopManage.emptyTitle')}</h2>
                    <p className="text-slate-500 dark:text-slate-400 mb-10 font-medium">{t('workshopManage.emptySubtitle')}</p>
                    <Link to="/workshops/manage/new" className="text-violet-600 font-black underline underline-offset-8">{t('workshopManage.emptyAction')}</Link>
                </div>
            ) : (
                            ...
                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                                <div className="flex items-start justify-between mb-2">
                                    ...
                                    <div className="flex gap-2">
                                        <button 
                                            ...
                                            title={t('workshopManage.manageReservations')}
                                        >
                                            <Users size={20} />
                                        </button>
                                        <button 
                                ...
                                <div className="flex flex-wrap items-center gap-4 mt-auto pt-4 border-t border-slate-50 dark:border-slate-800">
                                    ...
                                    <div className="flex items-center gap-1.5 text-slate-500">
                                        ...
                                        <span className="text-xs font-bold">{t('workshopManage.operating')}</span>
                                    </div>
            ...
            <div className="mt-12 bg-slate-50 dark:bg-slate-900/50 rounded-3xl p-8 border border-slate-100 dark:border-slate-800">
                <h4 className="text-lg font-black text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                    <AlertCircle size={20} className="text-violet-500" /> {t('workshopManage.noticeTitle')}
                </h4>
                <ul className="space-y-3 text-sm text-slate-600 dark:text-slate-400 font-medium">
                    <li className="flex gap-2">• {t('workshopManage.notice1')}</li>
                    <li className="flex gap-2">• {t('workshopManage.notice2')}</li>
                    <li className="flex gap-2">• {t('workshopManage.notice3')}</li>
                </ul>
            </div>
        </div>
    );
