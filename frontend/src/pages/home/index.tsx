...
import { MapPin, Star, Clock, ShoppingCart, RefreshCw, Zap, ArrowRight, Heart } from 'lucide-react';
...
const Home: React.FC = () => {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            {/* Hero Section */}
            <section className="relative overflow-hidden bg-white border border-slate-200 rounded-3xl p-10 mb-10 shadow-sm">
                ...
                <div className="relative z-10 max-w-2xl">
                    ...
                    <h1 className="text-5xl font-extrabold text-slate-900 mb-6 tracking-tight leading-tight">
                        일상이 예술이 되는 시간, <br />
                        <span className="text-violet-600 font-black italic">CraftDay</span>
                    </h1>
                    ...
                    <div key={region} className="flex-shrink-0 w-80">
                        <div className="flex items-center justify-between mb-4 px-2">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold text-slate-800">{region}</h2>
                        ...
                        <div className="flex flex-col gap-4">
                            {isLoading ? (
                                Array(2).fill(0).map((_, i) => (
                                    <div key={i} className="bg-slate-50 h-64 rounded-2xl border border-slate-200 animate-pulse"></div>
                                ))
                            ) : (
                                workshops
                                    .filter(w => w.region === region)
                                    .map(workshop => (
                                        <div key={workshop.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:border-violet-300 transition-all hover:shadow-lg group cursor-pointer">
                                            <div className="relative h-40 overflow-hidden">
                                                ...
                                                <div className="absolute top-3 left-3">
                                                    <span className="bg-white/90 backdrop-blur-sm text-violet-700 text-[10px] font-black px-2 py-1 rounded-lg shadow-sm uppercase">
                                                        {workshop.category}
                                                    </span>
                                                </div>
                                            ...
                                            <div className="p-5">
                                                ...
                                                <h3 className="text-slate-900 font-bold mb-1 line-clamp-1 group-hover:text-violet-600 transition-colors">
                                                    {workshop.title}
                                                </h3>
                                                <p className="text-slate-400 text-xs mb-4">{workshop.instructor}</p>
                                                <div className="flex items-center justify-between border-t border-slate-50 pt-4">
                                                    <div className="text-lg font-black text-slate-900">
                                                        <span className="text-xs font-medium text-slate-500 mr-1">₩</span>
                                                        {workshop.price}
                                                    </div>
                                                    <button className="p-2 bg-violet-50 text-violet-600 rounded-lg hover:bg-violet-600 hover:text-white transition-all">
                            ...
                            {!isLoading && workshops.filter(w => w.region === region).length === 0 && (
                                <div className="py-12 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 rounded-2xl">
                                    <p className="text-slate-300 text-sm font-medium">조회된 공방이 없습니다</p>
                                </div>
                            )}
                        </div>
                    </div>
