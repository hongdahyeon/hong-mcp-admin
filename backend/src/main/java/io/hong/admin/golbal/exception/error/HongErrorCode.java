...
public enum HongErrorCode {
    // 공통
    INTERNAL_SERVER_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "COMM-001", "서버 내부 오류입니다."),
    INVALID_INPUT_VALUE(HttpStatus.BAD_REQUEST, "COMM-002", "잘못된 입력값입니다."),

    // 인증/인가
    UNAUTHORIZED(HttpStatus.UNAUTHORIZED, "AUTH-001", "인증되지 않은 사용자입니다."),
    INVALID_TOKEN(HttpStatus.UNAUTHORIZED, "AUTH-002", "유효하지 않은 토큰입니다."),
    USER_NOT_FOUND_BY_TOKEN(HttpStatus.UNAUTHORIZED, "AUTH-003", "존재하지 않는 사용자 계정입니다."),
    AUTHENTICATION_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "AUTH-004", "인증 처리 중 오류가 발생했습니다."),
    EXPIRED_TOKEN(HttpStatus.UNAUTHORIZED, "AUTH-005", "만료된 토큰입니다."),

    // 유저 상태 관련 (상세화)
    USER_NOT_FOUND(HttpStatus.NOT_FOUND, "USER-001", "존재하지 않는 사용자입니다."),
    PASSWORD_NOT_MATCH(HttpStatus.UNAUTHORIZED, "USER-002", "비밀번호가 일치하지 않습니다."),
    USER_DELETED(HttpStatus.FORBIDDEN, "USER-003", "탈퇴한 계정입니다."),
    USER_LOCKED(HttpStatus.FORBIDDEN, "USER-004", "계정이 잠겼습니다. 관리자에게 문의바랍니다."),
    USER_NOT_APPROVED(HttpStatus.FORBIDDEN, "USER-005", "현재 승인 대기 중인 계정입니다."),
